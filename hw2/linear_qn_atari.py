...
from deeprl_hw2.lqn import LQNAgent
...
def main():  # noqa: D103
    ...
    parser.add_argument('--save', default='output/lqn', help='Directory to save model to')
    # flag to enable replay memory and target fixing 
    ...
    memory = None
    if args.memory:
        args.save = args.save + '_memory'
        memory = ReplayMemory(1000000, window_length=window)
    else:
        memory = ReplayMemory(1, window_length=window)
    ...
    dqn = LQNAgent(model, preprocessor, memory, policy, enable_replay=args.memory, num_actions=num_actions, gamma=.99, target_update_freq=10000, num_burn_in=50000, 
