...
def create_model(window, input_shape, num_actions, tuned=True,
    ...
    # keras Sequantial model. Same structure as DQN
    model = Sequential()
    ...
    # if tuned:
    #     model.add(Dense(num_actions, use_bias=False))
    #     model.add(SharedBias(num_actions))
    # else:
...
def main():  # noqa: D103
    ...
    parser.add_argument('--save', default='output/ddqn', help='Directory to save model to')
    parser.add_argument('--seed', default=0, type=int, help='Random seed')
    # add no_tuned flag for DQN parameters
    parser.add_argument('--no_tuned', action='store_false')
    parser.add_argument('--mode', choices=['train', 'test'], default='train')
    parser.add_argument('--load', default='output/ddqn', help='Directory to load model from')
    ...
    model = create_model(window, input_shape, num_actions, tuned=args.no_tuned)
    ...
    policy = LinearDecayGreedyEpsilonPolicy(GreedyEpsilonPolicy(epsilon=start_eps), attr_name='eps', start_value=start_eps, end_value=end_eps, num_steps=1000000)
    dqn = DDQNAgent(model, preprocessor, memory, policy, num_actions=num_actions, gamma=.99, target_update_freq=target_update_freq, num_burn_in=50000, 
                    train_freq=4, batch_size=32, output_dir=outputpath, tuned=args.no_tuned)
