...
class LQNAgent:
    ...
    def update_policy(self):
        ...
        if self.step > self.num_burn_in and self.step % self.train_freq == 0:
            ...
            target_q_values = []
            if self.enable_replay: # forward pass from target model
        ...
        # update weights of target model from trainable model if enable target fixing
        if self.enable_replay and self.target_update_freq >= 1 and self.step % self.target_update_freq == 0:
            self.target_model.set_weights(self.model.get_weights())
