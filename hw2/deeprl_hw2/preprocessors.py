...
class HistoryPreprocessor(Preprocessor):
    ...
    def process_state_for_network(self, state):
        ...
        if self.state_history is None:
            self.state_history = np.stack([np.zeros_like(state)] * (self.history_length - 1) + [state], axis=2)
        else:
            self.state_history = np.append(self.state_history[:,:,1:], np.expand_dims(state, 2), axis=2)
...
class AtariPreprocessor(Preprocessor):
    ...
    def process_state_for_memory(self, frame):
        ...
        if self.pre_frame is None:
            ...
            img = Image.fromarray(np.maximum(self.pre_frame, frame))
        self.pre_frame = frame
        return np.array(img.resize(self.input_shape).convert('L')).astype('uint8')
    ...
    def process_state_for_network(self, frame):
        ...
        if self.pre_frame is None:
            ...
            img = Image.fromarray(np.maximum(self.pre_frame, frame))
        self.pre_frame = frame
        return np.array(img.resize(self.input_shape).convert('L')).astype('float32') / 255
    ...
    def process_batch(self, samples):
        ...
        terminals = []
        for sample in samples:
            states.append(sample.state.astype('float32') / 255.)
            actions.append(sample.action)
            rewards.append(sample.reward)
            next_states.append(sample.next_state.astype('float32') / 255.)
