...
class LDQNAgent:
    ...
    def compile(self, optimizer, loss_func):
        ...
        # q_model1
        y_pred1 = self.model.output
        # mask layer. Share weight from trainable model
        y_masked_pred1 = Lambda(lambda y: y[0] * y[1], output_shape=(self.num_actions,), name='output')([y_pred1, y_mask])
        self.q_model1 = Model(inputs=[self.model.input, y_mask], outputs=y_masked_pred1)
        print(self.q_model1.summary())
        self.q_model1.compile(optimizer=optimizer, loss=losses, metrics=metrics)

        # q_model2
        y_pred2 = self.target_model.output
        # mask layer. Share weight from trainable model
        y_masked_pred2 = Lambda(lambda y: y[0] * y[1], output_shape=(self.num_actions,), name='output')([y_pred2, y_mask])
        self.q_model2 = Model(inputs=[self.target_model.input, y_mask], outputs=y_masked_pred2)
        print(self.q_model2.summary())
        self.q_model2.compile(optimizer=optimizer, loss=losses, metrics=metrics)
    ...
    def calc_q_values(self, state):
        ...
        # forward pass for prediction. Used average Q value to select action
        q_values = (self.model.predict_on_batch(process_state) + self.target_model.predict_on_batch(process_state)) / 2.
        return q_values
    ...
    def select_action(self, state, isTraining, **kwargs):
        ...
        selected action
        """
        q_values = self.calc_q_values(state[np.newaxis, :]).flatten()
        if isTraining:
    ...
    def update_policy(self):
        ...
        if self.step > self.num_burn_in and self.step % self.train_freq == 0:
            ...
            # coin-flip to select q model
            if np.random.uniform() < 0.5: # select q model 1
                best_actions = np.argmax(self.model.predict_on_batch(next_states), axis=1)
                target_q_values = self.target_model.predict_on_batch(next_states)
                max_target_q_values = target_q_values[np.arange(self.batch_size), best_actions]

                target_g_values = np.zeros((self.batch_size, self.num_actions))
                ...
                # backprop to update model 1
                metrics = self.q_model1.train_on_batch([states, masks], targets)
                self.loss.append(metrics)
            
            else: # select q model 2
                best_actions = np.argmax(self.target_model.predict_on_batch(next_states), axis=1)
                target_q_values = self.model.predict_on_batch(next_states)
                max_target_q_values = target_q_values[np.arange(self.batch_size), best_actions]
                ...
                # backprop to update model 2
                metrics = self.q_model2.train_on_batch([states, masks], targets)
                self.loss.append(metrics)
    ...
    def fit(self, env, num_iterations, callbacks=None, max_episode_length=None, episode_log_freq=20):
        ...
        self.evaluate(env.unwrapped, 100)
    ...
    def save_weights(self, filepath, overwrite=False):
        # save weights of both models to file
        self.model.save_weights(filepath, overwrite=overwrite)
        self.target_model.save_weights(filepath + '_target', overwrite=overwrite)
