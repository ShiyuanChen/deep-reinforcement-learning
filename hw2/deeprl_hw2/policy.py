...
class GreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, q_values, **kwargs):
        ...
        if np.random.uniform() < self.eps:
            return np.random.random_integers(0, q_values.shape[0] - 1)
        else:
            return np.argmax(q_values)
...
class LinearDecayGreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, **kwargs):
        ...
        Any:
          Selected action.
        """
        delta = (self.start_value - self.end_value) / float(self.num_steps)
        self.value = max(self.end_value, self.start_value - delta * float(self.agent.step))
        setattr(self.policy, self.attr_name, self.value)
